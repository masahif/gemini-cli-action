...
async function run() {
  try {
    const commentId = parseInt(process.env.GEMINI_COMMENT_ID!);
    const githubToken = process.env.GITHUB_TOKEN!;
    const geminiBranch = process.env.GEMINI_BRANCH;
  ...
  try {
        ...
        try {
          const { data: comparison } =
            await octokit.rest.repos.compareCommitsWithBasehead({
              owner,
              repo,
              basehead: `${baseBranch}...${geminiBranch}`,
            });
          ...
          if (
            ...
            const entityType = context.isPR ? "PR" : "Issue";
            const prTitle = encodeURIComponent(
              `${entityType} #${context.entityNumber}: Changes from Gemini`,
            );
            const prBody = encodeURIComponent(
              `This PR addresses ${entityType.toLowerCase()} #${context.entityNumber}\n\nGenerated with [Gemini Code](https://gemini.google.com/code)`,
            );
            const prUrl = `${serverUrl}/${owner}/${repo}/compare/${baseBranch}...${geminiBranch}?quick_pull=1&title=${prTitle}&body=${prBody}`;
            prLink = `\n[Create a PR](${prUrl})`;
          }
        } catch (error) {
          console.error("Error checking for changes in branch:", error);
    ...
    if (!prepareSuccess && prepareError) {
      ...
      try {
        ...
        // Check if the Gemini action failed
        const geminiSuccess = process.env.GEMINI_SUCCESS !== "false";
        actionFailed = !geminiSuccess;
      } catch (error) {
        console.error("Error reading output file:", error);
        // If we can't read the file, check for any failure markers
        actionFailed = process.env.GEMINI_SUCCESS === "false";
      }
    }
    ...
    const commentInput: CommentUpdateInput = {
      ...
      branchLink,
      prLink,
      branchName: shouldDeleteBranch ? undefined : geminiBranch,
      triggerUsername,
      errorDetails,
    };
